...
- 默认软预算 ¥50、硬预算 ¥100；不设默认项目墙钟完成时限。如用户显式设置“付费审美返工截止”，它也只停止新付费审美返工，不取代确定性收尾、QA 或完整交付门禁。心跳、取消、磁盘与预算门禁始终保留，超过硬预算不得发起新付费调用。
- 每次外部或模型请求必须记录 provider、request_id、model、开始/结束/等待、重试、Token 和估算/实扣成本状态；未报告值必须是 `null` 并标注原因，不得写 0 或 settled。
