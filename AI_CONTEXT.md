...
13. [`docs/architecture/module-ports.md`](docs/architecture/module-ports.md)：M3-3A 的 Port/Adapter/Quality Policy 边界、Registry 与 mock 注入。
...
| `story_module_ports.py` / `story_module_registry.py` / `story_module_adapters.py` | M3 版本化 Port kernel、显式 Registry、现有 Video/Keyer 薄适配器与离线 mock |
...
- M3-3A 已建立最小 Module Port kernel，并让正式视频 provider resolution/invocation seam 与 Demo/Release/keying evidence seam 分别经过 `VideoGeneratorPort`、`KeyerPort`；当前电器、DAG 和 Quality Policy 未改变。其余 Port、智能路由和 M4 账本/可观察性尚未实现。
