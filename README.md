...
> [`AI_CONTEXT.md`](AI_CONTEXT.md)。系统架构、阶段 DAG、模型路由、运行手册、
> 决策记录和真实项目复盘位于 [`docs/`](docs/)；不要从遍历全部历史文件开始理解工程。

当前正式可回退版本仍是 [`Story Agent V3 基线`](docs/baselines/V3_BASELINE_2026-08-12.md)。V3.5 开发分支已经完成 Milestone 1 合同前置和 Milestone 2 质量机制的工程实现；这只是开发安全点，不代表已合并 `main`、已发布候选版或已获用户产品验收。M2 收口事实见 [`V3.5 M2 Closeout`](docs/roadmaps/V3.5_M2_CLOSEOUT_2026-08-17.md)。

《小壁虎借尾巴》完成过工程交付和内部 QA，但用户终验暴露了画风、尺度、状态语义、动效、抠像、版式、封面和朗读指导问题；现有样本不重做，作为 V3 基准保留。V3.5 M2 已把这些问题转化为通用离线门禁与可追溯生产合同，真实 ImageGen/Grok/Suno、新人物素材和完整新故事仍等待小规模 canary 与用户验收。后续范围见 [`V3.5 质量修正与模块化升级`](docs/roadmaps/V3.5_QUALITY_AND_MODULARIZATION.md)。
...
它不会自动上传或发布。默认费用软上限 50 元、硬上限 100 元、运行时限 10 小时；到时冻结最佳哈希有效版本并停止新增审美返工。达到硬上限、磁盘不足、登录/CAPTCHA 或无法自动修复的问题时会安全阻塞并保留全部状态。
...
只有保留 `submit` 单绿幕输入契约、通过 `start` 后台无人值守启动、supervisor 启动记录与完成日志可验证、所有 Agent 阶段通过、审核产物不早于无人值守启动、全部审核哈希仍为当前版本、成本不超过 ¥50、运行时记录有效、人工终审通过且不超过 10 分钟的不同故事才会计数。达到 3/3 前，工作台仍保留为故障驾驶舱，Agent 不宣称已经转正。
