...
- 默认软预算 ¥50、硬预算 ¥100、运行时限 10 小时；到时冻结最佳哈希有效版本并停止新增审美返工，同时保留心跳、取消、磁盘与硬预算门禁。
...
- `99_项目状态` 固定生成 `成本报告.md/json`、`QA汇总.md/json` 和 `异常说明.md`；它们是内部交付，最终门槛要求存在，但不得复制进基础版或进阶版客户资料包。
- 资料包机器 QA 分别检查含/无字幕背景视频与含/无字幕 PPT，拒绝零字节文件和内部报告泄漏；不能再用单个“背景视频”或“故事PPT”关键词代替双版本。
- 发布物料机器 QA 检查六张封面的比例、最小尺寸和归一化图像差异，拒绝同一母版机械缩放；两份文案必须形成标题、正文和话题结构，之后才进入独立视觉审核。
