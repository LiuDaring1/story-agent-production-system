...
| `story_pipeline.py` / `story_run.json` | 记录六个粗粒度工作包、当前有效产物、SHA-256、供应商任务号与金额，并在最后封口 | 不导演、不调度、不自动恢复某个阶段 |
...
| 8A | R2V jobs | provider adapter + ToAPIs Grok 1.0 runner | 逐镜任务号、实际提示词、下载 MP4、状态和费用 | Codex：完整性检查→连续预览→有证据的返工；随后独立整组审核 | jobs CSV、provider receipts、QA、review/bundle |
...
1. `story_run.json`：项目级摘要——六包状态、当前有效产物、哈希、任务号、金额、是否封版。
