...
| 角色 | 默认模型 | 当前推理档位 | 责任 |
| --- | --- | --- | --- |
| 指挥官 | `gpt-5.6-sol` | Medium | 关键判断、视觉审核、发布审核、产品审核 |
| 工人 | `gpt-5.6-luna` | High | 生产、工具执行、图片、发布资产和一般阶段 |
...
当前档位是《七色花》短测后的候选基线，不是永久结论。未来应通过相同代表镜头进行受控实验，比较：

- Luna Medium 与 Luna High；
- Terra Medium；
- 必要时的 Sol High/X-high。

指标必须是“合格产物的总成本”：首轮通过率、返工次数、墙钟时间、Token 和人工干预，而不是只比较单次响应速度。

建议升级梯度：确定性脚本不用模型；普通生产使用 Luna High；连续失败或复杂判断才定点升级；最终关键审核保留 Sol Medium/High。Max/X-high 不作为默认值。
