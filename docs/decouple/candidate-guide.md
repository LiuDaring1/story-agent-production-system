...
本版用于后续新故事试生产，尚未晋级稳定默认版。沿用上面的显式 v2 输入、任务绑定及取消/恢复方法；不自动重编码任何历史项目。

编码缓存按实际编号图片序列的有序成员路径及 SHA-256 建立指纹，位宽为最小宽度（例如 %03d 包含 1000）。新增、删除、改图和编码期间成员变化均会被发现；同路径字幕 PNG 更新会使字幕层更新，并影响消费该层的下游指纹。匹配范围采取保守全集，可能包含当前 -t/start_number 之外的成员，从而多重编码，但不会据此漏掉变化。

直接文件、编号序列及已识别自包含参数可以复用；拼接清单（包括内容被识别为清单但后缀是视频的文件）、滤镜图/外部脚本、协议输入、glob 排序及未知格式或参数，暂时禁用复用并正常调用编码器。回执 dependencies.no_reuse_reasons 说明原因。脚本本体及可解析的拼接引用同时记录哈希；不把未解析依赖当成“未变化”。这是正确性优先的保守策略，某些未变化的复杂合成会重新编码，未宣称提速。
