# 已归档：工作台、合成器与 Story Agent 历史 README

> 本文仅用于历史项目复现，不定义现行生产入口。当前入口是 Codex 前台任务 + `skills/story-full-auto`，内部状态入口是 `story_pipeline.py`。

> AI 或新维护者请先阅读 [`AGENTS.md`](AGENTS.md) 和
> [`AI_CONTEXT.md`](AI_CONTEXT.md)。当前架构见
> [`Codex 原生故事生产架构`](docs/architecture/codex-native-story-pipeline.md)；不要从旧阶段文档或历史媒体目录开始理解工程。

当前唯一生产入口是一个 Codex 前台任务，并由 [`story-full-auto`](skills/story-full-auto/SKILL.md) 提供生产策略。Codex 负责导演判断、观察、调度和异常处置；`story_pipeline.py` 只负责持久状态、当前有效产物和最终封口；实际执行由小型 Python/JS 模块及供应商适配器完成。

旧 `story_agent.py`、38 阶段 Runtime、Supervisor、Dashboard 和 Recovery 已归档到 [`legacy/story_agent_v3`](legacy/story_agent_v3/LEGACY.md)。根目录同名模块只是历史导入兼容层，不能作为新故事入口。旧 V3 文档、基线和测试继续保留作为工程教训和迁移证据。

## 正常使用

在 Codex 中提供用户确认的故事文本和已调色横屏绿幕视频，并要求使用 `$story-full-auto` 生产新故事。不要启动 `story_agent.py`、旧 Dashboard 或工作台阶段循环。

Codex 使用以下内部入口记录和恢复状态：

```bash
python3 story_pipeline.py describe
python3 story_pipeline.py init --run-file ... --project-dir ... --text ... --video ... --audio ...
python3 story_pipeline.py status --run-file .../story_run.json
```

这不是固定 DAG。导演镜头、已审核资产、逐镜故事板、R2V、音乐、RVM、发布美术、资料包和封面按当前结果动态推进，并以 manifest、独立审核、QA 和 SHA-256 回执汇合。默认费用软上限 ¥50、硬上限 ¥100；系统不会自动上传或发布。

下面内容以及 [`STORY_AGENT_SPEC.md`](STORY_AGENT_SPEC.md) 属于历史合成器/工作台/Story Agent 兼容说明，不定义当前生产架构。
