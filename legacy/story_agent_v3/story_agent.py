    ...
    def _can_retry_stage(
        ...
        *,
        ...
        if retry_key in defaults:
            ...
            # Legacy 自己保留历史默认值；新原生配置不再携带或消费这些字段。
            legacy_attempts_key = "max_critical_attempts" if critical else "max_attempts"
            legacy_default_limit = 4 if critical else 3
            limit = int(defaults.get(legacy_attempts_key, legacy_default_limit))
        attempts = (
