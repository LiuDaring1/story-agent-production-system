...
- 必须分别生成 `cover_3x4.png`、`cover_4x3.png`、`cover_16x9.png`；每种比例重新组织版式，禁止机械裁切或拉伸。

发布/预览参考帧：`{reference_image}`

宝库号封面逻辑：保留故事、标题和资源信息，但重新生成一张统一的 4:3 横版森林故事封面，不要照搬预览帧里的框、卡片或分栏模板。参考帧只用于理解故事角色、场景氛围和资料包信息；如果只是标题页、空书房、纯资料展示、普通书卷景，必须回候选帧重新选择。
...
## 生成方式硬性要求
- 必须回到 Codex 对话中使用 Codex 原生图像生成能力生成最终三张封面。
- 必须分别生成 `cover_3x4.png`、`cover_4x3.png`、`cover_16x9.png`；三种比例都保持真人一致性，禁止机械裁切。
...
主账号封面逻辑：以第 13 步已确认的发布预览帧或发布成片截屏为参考，分别为 3:4、4:3、16:9 重新构图。人物一致性优先，不能把真人绿幕帧重新捏成另一个人。
...
- 服装保持浅色连衣裙/浅色上衣、胸前黑色麦克风，人物可以重新进入故事场景，但身份必须像参考图本人。
...
## 画面方向
- 故事视觉元素：{visual_elements}。
- 中国风儿童故事节目包装，明亮、显眼、适合小红书/视频号封面。
- 画面必须是一张统一的森林故事场景封面：人物、标题、故事角色和背景自然在同一个空间里。
- 标题要大，人物和故事画面要自然融合，不要像把截屏硬贴在模板里。
- 资料包信息可以放底部同一条信息带，但禁止做成六宫格、白色资源卡、内嵌小画面、木质大相框或分栏模板。
...
- 每种比例独立组织人物、标题和资料信息，禁止机械裁切、拉伸或补边。

封面参考帧：优先使用 `reference_3x4.png`；如果不存在，则从第 13 步发布预览帧中选择主账号画面稳定的一帧。

请以发布预览帧里的实际人物、故事角色和标题信息为参考，分别生成三种比例的封面。不要重新换人物，不要换故事角色；不要照搬预览里的木质相框、截图框或模板分区。
...
def write_manifest(output_dir: Path, story_name: str, accounts: tuple[AccountSpec, ...], generated_covers: bool) -> None:
    lines = [
        ...
        "- 六张封面必须使用 Codex 原生生图能力，每个比例独立构图。",
    ...
    if generated_covers:
        lines.extend(
            [
                ...
                "4. 六张封面必须按各自比例独立构图；禁止用 Pillow、HTML/CSS、截图拼接、模板叠字或其他本地脚本合成最终封面。",
            ]
        )
    else:
        lines.extend(
...
def write_codex_handoff(
    *,
    ...
    lines = [
        ...
        "",
        "请继续处理这个故事的全自动发布物料。生成主账号/宝库号两份文案，并为每个账号生成 3:4、4:3、16:9 三张封面，共六张；封面必须使用 Codex 原生生图能力且每个比例独立构图。",
        "",
        ...
        "2. 宝库号封面：基于能体现故事关键情节或角色动作的发布视频帧生成三种比例，不能使用标题页、空景或纯资料展示帧。",
        "3. 主账号封面：基于已确认的主账号预览帧和真人/故事参考生成三种比例。",
        "4. 主账号真人一致性优先：生成时尽量保留预览帧中原主持人的脸型、五官比例、发型、服装和姿态。",
        "5. 3:4、4:3、16:9 必须分别适配安全区，不能机械裁切。",
        "6. 生成后可用脚本做尺寸标准化和复制落位，但不能用脚本重新画版式、叠文字或拼合最终封面。",
        "",
        "## 目标保存路径",
