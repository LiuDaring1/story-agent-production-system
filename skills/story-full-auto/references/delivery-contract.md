...
Agent 交付：主账号和宝库号视频；基础版含原 Word、原音乐、Demo、背景原图；进阶版含基础版四项、含/无字幕背景视频、A 镜背景视频、PPT 素材普通文件夹。文件夹含 TITLE/shot_id/可选 MORAL 同源图、原旁白、原音乐、顺序/文字/权威时间/哈希清单。用户后来加入的文件不属于完成门禁，不要求目录总数等于清单数。

商品宣传内容单独来自用户确认，PPT与朗读标注可由用户提供；不得用上述Agent交付清单推断整个商品。Agent素材目录不等于用户最终PPT。
...
最终审核 bundle 覆盖当前清单、全部成员、发布 QA；分数 >=85、critical_errors=[]，并保留合并审核的音频、字幕、Logo、人物几何证据。文件存在/状态完成不能替代 QA。自有资料包 Word/音乐须与输入 SHA 完全一致，材料目录按有序清单逐成员校验，目录摘要绑定相对路径、成员SHA及字节数；ZIP仅显式可选，旧v2 ZIP回执只读兼容。
