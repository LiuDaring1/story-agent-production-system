...
def _package_locked(*, output_root, receipt, sources, story_name, inputs):
    ...
    names = {'customer_manuscript': '故事文稿', 'music': '故事配乐', 'demo': '示范表演', 'background_image': '背景图片', 'background_video_with_subtitles': '背景视频（含字幕）', 'background_video_without_subtitles': '背景视频（无字幕）', 'a_only_video': 'A镜无人物背景视频', 'ppt_materials': 'PPT素材'}
    planned = []
    for variant, label, roles in [('base', '基础版', BASE_ROLES), ('advanced', '进阶版', ADVANCED_ROLES)]:
        ...
        for role in roles:
            ...
            dest = directory / f'{names[role]}：{story_name}{source.suffix if source.is_file() else ""}'
    ...
    for old_path, item in old.items():
        ...
        if not source.exists() or binding(source)['sha256'] not in {item['sha256'], item.get('prior_sha256')}:
    ...
    for item in retired:
        ...
        elif not source.exists() or binding(source)['sha256'] != item['sha256']:
            raise ValueError(f'User changed retired managed file; preserve it: {source}')
    pending = {'schema_version': 'story-managed-package/v2', 'status': 'copying', 'retired': retired,
               'artifacts': [{'role': f'{variant}:{role}', **{**bound[role], 'path': str(dest)}, 'prior_sha256': binding(dest)['sha256'] if dest.exists() else None, 'source': bound[role]} for variant, role, source, dest in planned]}
    ...
    for variant, role, source, dest in planned:
        ...
        if not dest.exists() or binding(dest)['sha256'] != bound[role]['sha256']:
            ...
            if source.is_dir():
                if dest.exists():
                    raise ValueError('Changed materials directory needs a new package version; preserve existing files')
    ...
    return {**payload, 'reused': copies_performed == 0}
