...
class ModuleRegistry:
    ...
    def product_package(self) -> ProductPackagePort:
        return self.get("product_package")  # type: ignore[return-value]
    ...
    def publish_asset(self) -> PublishAssetPort:
        return self.get("publish_asset")  # type: ignore[return-value]
...
def build_default_registry(
    ...
    *,
    ...
    registry.register("publish_asset", LocalPublishAssetAdapter())
    return registry
...
def build_product_package_registry(profile: str = "") -> ModuleRegistry:
    """Build the local-only product-package registry without provider config."""
...
def build_compositor_registry(profile: str = "") -> ModuleRegistry:
    """Build the local-only compositor registry without provider config."""
...
def build_publish_asset_registry(profile: str = "") -> ModuleRegistry:
    """Build the local-only publish-asset registry without provider config."""
