...
def _generic_action_from_story(source: str) -> str:
    ...
    if any(token in source for token in ("吃", "咬", "舔")):
        return f"{subject}对原图中的食物做轻微咬食动作，随后舔舔嘴；食物只按故事情节少量变化，其他角色和构图保持稳定"
...
def _infer_subject(source: str) -> str:
    for subject in ("小狗", "珍妮", "猫", "老鼠", "小猫", "小老鼠", "小羊", "狐狸", "兔子", "熊", "小熊", "鹿", "猴子", "孙敬", "苏秦", "主角"):
