...
def run(output):
    ...
    person_png=output/'person.png'; im=Image.new('RGBA',(320,180));ImageDraw.Draw(im).rectangle((220,30,270,179),fill=(220,35,55,255));im.save(person_png)
    person=output/'foreground.webm';ff('-loop',1,'-framerate',5,'-i',person_png,'-t',duration+.2,'-c:v','libvpx-vp9','-lossless',1,'-pix_fmt','yuva420p','-auto-alt-ref',0,person)
    ...
    projection=output/'projection.json';projection.write_text('{}');ledger['artifacts']['requirements_projection']=binding(projection)
    ...
    geometry={'bindings':{},'schema_version':'fixture','geometry_sha256':'fixture',
              'presenter':{'a':{'source_crop':[0,0,320,180],'rendered_width':1920,'rendered_height':1080,'x':0,'y':0},
    ...
    with patch.object(sys,'argv',argv),patch('story_run.load_run',return_value=ledger),\
         patch.object(release,'preflight_release_requirements'),\
         patch.object(release,'load_keying_preset',return_value={'keyer':'rvm','rvm_foreground_video':str(person),'person_grade':'none'}),\
         ...
         patch('story_media_preview.load_approved',return_value=({'preview':binding(raw_preview)},{'keying_lock_sha256':binding(lock)['sha256']})),\
