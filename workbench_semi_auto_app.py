...
from story_project import create_theme_asset_request, ensure_project_dirs, load_config, project_paths, register_manual_output


class StoryWorkbenchApp:
    def __init__(self, root: Tk) -> None:
        self.root = root
        self.root.title("儿童故事生产工作台｜半自动高效版")
        ...
        self.manual_no_sub_bg_path = StringVar()
        self.manual_with_sub_bg_path = StringVar()
        self.manual_demo_voice_path = StringVar()
        self.manual_main_release_path = StringVar()
        self.manual_library_release_path = StringVar()
        self.manual_a_scene_no_person_path = StringVar()
        self.manual_annotation_path = StringVar()
        self.manual_ppt_path = StringVar()
    ...
    def _build_ui(self) -> None:
        ...
        semi = LabelFrame(left, text="半自动高效版：人工优先登记与并行指引")
        semi.pack(fill=X, pady=(10, 0))
        semi_grid = Frame(semi)
        semi_grid.pack(fill=X, padx=8, pady=8)
        semi_buttons = [
            ("刷新半自动状态", self.semi_auto_status),
            ("登记背景成片三版", self.register_background_videos),
            ("登记主账号发布视频", self.register_main_release_video),
            ("登记宝库号发布视频", self.register_library_release_video),
            ("登记A镜无人版", self.register_a_scene_no_person),
            ("登记外部朗读标注", self.register_annotation),
            ("登记PPT/图片素材", self.register_ppt_asset),
            ("手工产物检查", self.qa_manual_outputs),
            ("打开输入素材", lambda: self.open_project_subdir("inputs")),
            ("打开背景成片", lambda: self.open_project_subdir("assembly")),
            ("打开发布视频", lambda: self.open_project_subdir("release")),
            ("打开资料包_manual", lambda: self.open_project_subdir("product_manual")),
        ]
        for index, (text, command) in enumerate(semi_buttons):
            button = Button(semi_grid, text=text, command=lambda action=command: self._guard(action), height=2)
            button.grid(row=index // 4, column=index % 4, sticky="ew", padx=4, pady=4)
            semi_grid.columnconfigure(index % 4, weight=1)

        guide = LabelFrame(left, text="当前建议 / 可并行任务")
        guide.pack(fill=X, pady=(10, 0))
        self.guide_text = Text(guide, height=9, wrap="word", state=DISABLED)
        self.guide_text.pack(fill=X, padx=8, pady=8)
        ...
        asset_box = LabelFrame(right, text="素材识别与命名提醒")
        asset_box.pack(fill=X, pady=(10, 0))
        self.asset_status_text = Text(asset_box, height=11, wrap="word", state=DISABLED)
        self.asset_status_text.pack(fill=X, padx=8, pady=8)
    ...
    def generate_videos(self) -> None:
        self._refresh_paths()
        self._require(self.jobs_csv(), "请先准备任务。")
        if not self._has_video_api_key():
            return
        ...
        if not prompt_review_csv.exists():
            ...
            return
        self.log("半自动版会先批量提交全部待生成镜头，再逐个轮询下载，减少服务端排队造成的串行等待。")
    ...
    def publish_package_project(self) -> None:
        ...
        try:
            ...
            pass
        self.log("第 15 步开始生成发布物料草稿任务书；半自动版不再强制等待主账号/宝库号视频都完成。")
        self._run_workflow(["publish-package-draft-project", "--project-dir", self.desktop_project_dir.get()])

    def semi_auto_status(self) -> None:
        self._refresh_paths()
        project_dir = Path(self.desktop_project_dir.get()).expanduser()
        report = project_dir / "99_项目状态" / "semi_auto_status.md"
        self.open_after_done = report
        self._run_workflow(["semi-auto-status", "--project-dir", self.desktop_project_dir.get()])

    def qa_manual_outputs(self) -> None:
        self._refresh_paths()
        project_dir = Path(self.desktop_project_dir.get()).expanduser()
        report = project_dir / "99_项目状态" / "qa_manual_outputs_report.md"
        self.open_after_done = report
        self._run_workflow(["qa-manual-outputs", "--project-dir", self.desktop_project_dir.get()])

    def register_background_videos(self) -> None:
        self._refresh_paths()
        selections = [
            ("background_video_no_sub", "选择背景视频无字幕版 story_no_subs_bgm.mp4", self.manual_no_sub_bg_path),
            ("background_video_with_sub", "选择背景视频有字幕版 story_sales_subs_bgm.mp4", self.manual_with_sub_bg_path),
            ("demo_voice_bgm", "选择人声+音乐示范版 story_demo_voice_bgm.mp4", self.manual_demo_voice_path),
        ]
        for kind, title, variable in selections:
            selected = filedialog.askopenfilename(
                title=title,
                initialdir=str(project_paths(Path(self.desktop_project_dir.get()).expanduser()).assembly),
                filetypes=[("视频", "*.mp4 *.mov *.m4v"), ("所有文件", "*.*")],
            )
            if selected:
                variable.set(selected)
                self._register_manual_output(kind, Path(selected))
        self.semi_auto_status()

    def register_main_release_video(self) -> None:
        self._select_and_register_video("main_release_video", "选择主账号发布视频", self.manual_main_release_path)

    def register_library_release_video(self) -> None:
        self._select_and_register_video("library_release_video", "选择宝库号发布视频", self.manual_library_release_path)

    def register_a_scene_no_person(self) -> None:
        self._select_and_register_video("a_scene_no_person_video", "选择A镜无人版视频", self.manual_a_scene_no_person_path)

    def register_annotation(self) -> None:
        self._refresh_paths()
        selected = filedialog.askopenfilename(
            title="选择外部朗读标注 .json 或 .docx",
            initialdir=str(project_paths(Path(self.desktop_project_dir.get()).expanduser()).product),
            filetypes=[("朗读标注", "*.json *.docx"), ("所有文件", "*.*")],
        )
        if selected:
            self.manual_annotation_path.set(selected)
            self._register_manual_output("annotation_file", Path(selected))
            self.semi_auto_status()

    def register_ppt_asset(self) -> None:
        self._refresh_paths()
        selected = filedialog.askopenfilename(
            title="选择PPT或PPT图片素材",
            initialdir=str(project_paths(Path(self.desktop_project_dir.get()).expanduser()).product),
            filetypes=[("资料素材", "*.ppt *.pptx *.pdf *.zip *.png *.jpg *.jpeg"), ("所有文件", "*.*")],
        )
        if selected:
            self.manual_ppt_path.set(selected)
            self._register_manual_output("ppt_file", Path(selected))
            self.semi_auto_status()

    def _select_and_register_video(self, kind: str, title: str, variable: StringVar) -> None:
        self._refresh_paths()
        selected = filedialog.askopenfilename(
            title=title,
            initialdir=str(project_paths(Path(self.desktop_project_dir.get()).expanduser()).release),
            filetypes=[("视频", "*.mp4 *.mov *.m4v"), ("所有文件", "*.*")],
        )
        if not selected:
            return
    ...
    def _register_manual_output(self, kind: str, source: Path) -> None:
        manifest = register_manual_output(Path(self.desktop_project_dir.get()).expanduser(), kind, source)
        saved = manifest.get("manual_outputs", {}).get(kind, "")
        self.log(f"已登记手工产物：{kind} -> {saved}")
        self._refresh_paths()
        self._load_manifest_to_ui()

    def open_project_subdir(self, key: str) -> None:
        self._refresh_paths()
        paths = project_paths(Path(self.desktop_project_dir.get()).expanduser())
        mapping = {
            "inputs": paths.inputs,
            ...
            "product_manual": paths.product / "_manual",
        }
        target = mapping[key]
    ...
    def assemble_final(self) -> None:
        ...
        script_path = final_dir / "script_lines.txt"
        if not clips_dir.exists() or not script_path.exists():
            messagebox.showinfo("请先应用审核", "请先点击“⑧ 应用审核”，生成最终片段 clips 和逐行文本 script_lines.txt 后，再点击“⑪ 机器合成背景成片”。")
            return
        command = [
            "assemble",
            "--video-dir",
    ...
    def _task_label(self, args: list[str]) -> str:
        labels = {
            ...
            "doctor-project": "工程体检",
            "semi-auto-status": "刷新半自动状态",
            "register-output": "登记手工产物",
            "qa-manual-outputs": "手工产物检查",
            "publish-package-draft-project": "发布物料草稿任务书",
        }
    ...
    def _refresh_paths(self) -> None:
        ...
        self._update_semi_auto_panels(paths)
    ...
    def _load_manifest_to_ui(self) -> None:
        ...
        for key, variable in (
            ("background_video_no_sub", self.manual_no_sub_bg_path),
            ("background_video_with_sub", self.manual_with_sub_bg_path),
            ("demo_voice_bgm", self.manual_demo_voice_path),
            ("main_release_video", self.manual_main_release_path),
            ("library_release_video", self.manual_library_release_path),
            ("a_scene_no_person_video", self.manual_a_scene_no_person_path),
            ("annotation_file", self.manual_annotation_path),
            ("ppt_file", self.manual_ppt_path),
        ):
            if manual.get(key):
                variable.set(str(manual[key]))
        self._update_semi_auto_panels(paths, manifest)

    def _update_semi_auto_panels(self, paths, manifest: dict | None = None) -> None:
        if manifest is None and paths.manifest.exists():
            try:
                manifest = json.loads(paths.manifest.read_text(encoding="utf-8"))
            except Exception:
                manifest = None
        ...
        manual = manifest.get("manual_outputs", {})
        slug = story.get("slug") or self.slug.get().strip() or "story"
        ...
        asset_lines = [
            "推荐把原始输入放入 `00_输入素材`；项目根目录也能识别，但不推荐。",
            "",
            "建议命名：",
            f"- 故事原文：{slug}_source.txt",
            f"- 原声/旁白：{slug}_narration.wav/mp3/m4a",
            f"- 16:9绿幕横屏原视频：{slug}_greenscreen.mp4/mov",
            f"- 背景音乐：{slug}_music.mp3/wav/m4a",
            "",
            "当前识别：",
            f"- 故事原文：{mark(inputs.get('story_text'))}",
            f"- 原声/旁白：{mark(inputs.get('narration') or inputs.get('extracted_narration'))}",
            f"- 绿幕视频：{mark(inputs.get('greenscreen_video'))}",
            f"- 背景音乐：{mark(inputs.get('music'))}",
            "",
            "手工产物：",
            f"- 背景无字幕：{mark(outputs.get('background_video_no_sub') or manual.get('background_video_no_sub'))}",
            f"- 背景有字幕：{mark(outputs.get('background_video_with_sub') or manual.get('background_video_with_sub'))}",
            f"- 主账号发布视频：{mark(outputs.get('main_release_video') or manual.get('main_release_video'))}",
            f"- 宝库号发布视频：{mark(outputs.get('library_release_video') or manual.get('library_release_video'))}",
            f"- 外部朗读标注：{mark(manual.get('annotation_file'))}",
        ]
        self.asset_status_text.configure(state=NORMAL)
        self.asset_status_text.delete("1.0", END)
        self.asset_status_text.insert("1.0", "\n".join(asset_lines))
        self.asset_status_text.configure(state=DISABLED)

        current_stage = manifest.get("semi_auto", {}).get("current_stage") or self._infer_ui_stage(inputs, outputs, manual)
        guide_lines = [
            f"当前阶段：{current_stage}",
            "",
            self._guide_for_stage(current_stage),
            "",
            "等待期间可并行做：",
            "- 出图中：生成配乐任务、去 Suno 做音乐、生成发布视觉素材任务书、准备达芬奇抠像。",
            "- 图生视频中：做 PPT、整理外部朗读标注、检查主账号背景图和宝库号底板图。",
            "- 背景成片导出后：回工作台登记三版背景视频，再继续发布视频和资料包。",
            "",
            "关键提醒：背景成片和主账号发布视频默认人工优先；机器合成按钮只是备用。",
        ]
    ...
    def _infer_ui_stage(self, inputs: dict, outputs: dict, manual: dict) -> str:
        if not inputs.get("story_text") or not inputs.get("narration"):
            return "项目与素材"
        if not outputs.get("storyboard"):
            return "原文与出图"
        ...
        if not outputs.get("publish_package"):
            return "发布物料"
        if not outputs.get("product_advanced"):
            return "资料包"
        return "最终交付"

    def _guide_for_stage(self, stage: str) -> str:
        mapping = {
            "项目与素材": "先创建项目，把故事原文、原声和绿幕视频放进 `00_输入素材`，然后点“保存并识别”。",
            "原文与出图": "确认右侧故事原文已经按分镜换行，再生成 Codex 出图任务；出图等待时可以先做音乐和发布视觉素材。",
            "图生视频准备": "图片完成后点“准备图生视频”和“写入旁白时长”。旁白时长来自原声，不来自绿幕视频。",
            "手工背景成片": "视频片段齐后，优先去达芬奇手工排镜头、慢放、加字幕，导出三版背景成片后回来登记。",
            "主账号发布视频": "用背景成片、绿幕视频、主账号背景图和透明框制作主账号发布视频；宝库号可手工或机器生成。",
            "发布物料": "现在可以提前生成发布物料草稿；即使发布视频还没齐，也能先做文案和横版封面方向。",
            "资料包": "正式打包前登记外部朗读标注、A镜无人版、背景有/无字幕版和示范视频。",
            "最终交付": "运行最终交付清单，逐项检查主账号、宝库号、发布物料和资料包。",
        }
        return mapping.get(stage, "刷新半自动状态，按缺项补齐素材。")
    ...
    def _codex_image_request_text(
        ...
        *,
        ...
        slug = self.slug.get().strip()
        request = build_children_story_image_request(
...
- 分镜脚本文本必须使用用户提供的故事原文。
- 不得改写、浓缩、润色或替换旁白文本。
- 每个镜头的画面可以重新设计，但镜头对应的故事文字必须来自 `{source_path}` 或 `{storyboard_path}`。
- 如果用户在工作台里已经手动换行，请以 `{storyboard_path}` 的逐行文本作为镜头文字基准。
- 如需画面内出现文字，只能按用户明确指定的文字出现，不要自行添加字幕、水印或额外标题。
"""
        return request.rstrip() + strict_text_rule
